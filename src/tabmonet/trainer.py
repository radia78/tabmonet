...
from tabmonet.schedulers.wd_scheduler import WDScheduler
...
class Trainer:
    ...
    def train_step(self, X, y):
        ...
        outputs, loss = self.model(cont_features, cat_features, targets)
        loss.backward()
    ...
    def val_epoch(self, data: DataLoader, epoch: Optional[int] = None):
        ...
        """ if epoch and self.problem_type == "multiclass":
            ...
            self.writer.add_figure("Plot", fig, epoch) """
    ...
    def run(
        ...
        log.info(f"Best Validation Score: {self.best_val_score:.4f}")
        log.info(f"Best Test Score: {test_metric:.4f}")
